...
def build_efficientnet_b0(pretrained=True):
    ...
    # Remove the classifier and avgpool, keep only features
    # EfficientNet-B0 features output: 1280 channels at 7x7 spatial resolution
...
class SAMPNet(nn.Module):
    def __init__(self, cfg, pretrained=True):
        ...
        backbone_type = getattr(cfg, 'backbone_type', 'resnet18')
        
        if backbone_type == 'efficientnet_b0':
            self.backbone = build_efficientnet_b0(pretrained=pretrained)
            input_channel = 1280  # EfficientNet-B0 output channels
        else:
            # Fall back to ResNet
