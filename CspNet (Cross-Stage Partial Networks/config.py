...
class Config:
    ...
    dataset_path = 'C:/Users/segov/Music/SAMP-Net/CADB_Dataset'
    assert os.path.exists(dataset_path), dataset_path + 'not found'

    batch_size = 4  # Modified as requested
    gpu_id = 0
    num_workers = 0  # Modified as requested

    # setting for training and optimization
    max_epoch = 50  # Modified as requested
    save_epoch = 1
    display_steps = 10
    score_level = 5
    ...
    lr = 1e-4  # Modified as requested
    weight_decay = 5e-5
    momentum = 0.9
    
    # setting for cnn - CSPNet specific
    image_size = 224
    cspnet_variant = 'cspdarknet53'  # Options: 'cspdarknet53', 'cspresnet50', 'cspresneXt50'
    dropout = 0.5
    pool_dropout = 0.5
    ...
    if len(pattern_list) == 1:
        ...
        prefix = 'cspnet_{}'.format(cspnet_variant)
        if use_multipattern:
